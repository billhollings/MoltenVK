...
typedef struct {
	...
	/**
	 * Controls whether MoltenVK should embed the contents of inline-block descriptors directly
	 * in the Metal argument buffer, instead of writing the contents of the descriptor in an
	 * intermediary MTLBuffer, which is then inserted into the Metal argument buffer. Embedding
	 * inline-block descriptor content directly into the Metal argument buffer improves efficiency
	 * and reduces resources, but currently does not cover all types of possible inline content,
	 * and may cause errors in some cases. If this setting is enabled, MoltenVK will embed
	 * inline-block descriptor content directly into the Metal argument buffers. If this setting
	 * is disabled, MoltenVK will write inline-block content to an intermediary MTLBuffer,
	 * and then insert that MTLBuffer into the Metal argument buffer. This setting only has
	 * effect if the useMetalArgumentBuffers setting is enabled and is supported.
	 *
	 * The value of this parameter must be changed before creating a VkInstance,
	 * for the change to take effect.
	 *
	 * The initial value or this parameter is set by the
	 * MVK_CONFIG_EMBED_INLINE_BLOCKS_IN_METAL_ARGUMENT_BUFFER
	 * runtime environment variable or MoltenVK compile-time build setting.
	 * If neither is set, this setting is disabled by default, and MoltenVK
	 * will use an intermediary MTLBuffer.
	 */
	VkBool32 embedInlineBlocksInMetalArgumentBuffers;
